...
"for(const k in heads){const e=$('th_'+k);if(e)e.innerHTML=heads[k]+(sortKey===k?(sortDir>0?' ▲':' ▼'):'');}"
"const arr=[...lastData];if(sortKey)arr.sort((a,b)=>{const x=a[sortKey],y=b[sortKey];return (sortKey==='score'?(x-y):String(x).localeCompare(String(y),'zh'))*sortDir;});"
"$('tb').innerHTML=arr.map(r=>'<tr><td>'+r.id+'</td><td>'+r.name+'</td><td>'+r.className+'</td><td>'+r.term+'</td><td>'+r.course+'</td>'+tdScore(r.score)"
"+(isAdmin?'<td><button class=\\'gray\\' onclick=\\'upd(\\\"'+r.id+'\\\",\\\"'+r.term+'\\\",\\\"'+r.course+'\\\",'+r.score+')\\'>改</button> '"
"+'<button class=\\'red\\' onclick=\\'del(\\\"'+r.id+'\\\",\\\"'+r.term+'\\\",\\\"'+r.course+'\\\")\\'>删</button></td>':'<td>-</td>')"
"+'</tr>').join('')||'<tr><td colspan=7>无记录</td></tr>';}"
"function clearFilter(){sortKey='';['f_id','f_name','f_class','f_term','f_course'].forEach(i=>$(i).value='');loadList();}"
